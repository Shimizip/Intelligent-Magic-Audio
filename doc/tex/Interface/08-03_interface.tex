...
\textbf{Teillösung:} \\
Das Lesen der Audiodateinamen und die Übertragung in den \boldinline{FileManager} funktionierten und ermöglichten somit eine Visualisierung auf dem Display.

Das Rausholen der Namen könnte über \boldinline{FILINFO fno} in der Methode \mintinline{c}|listFiles(const char *path)| realisiert werden, indem man als Pfad den im Root-Verzeichnis befindlichen Ordner \textbf{"/SamplePool"} übergeben hat.


 \inputminted[firstline=373, lastline=387]{c}{../../f401_display_encoder_fader_test/Core/Src/interface.c}
 
 Das \boldinline{fileNamesSDCard} würde bei der Initialisieung des Displays weiter verwendet. 
